...
ContributorView.Prototype = function() {
  ...
  this.renderBody = function() {
    ...
    var affiliations = this.node.getAffiliations();

    if (affiliations.length > 0) {
      this.content.appendChild($$('.affiliations', {
        children: _.map(affiliations, function(aff) {
    ...
    if (this.node.emails && this.node.emails.length > 0) {
      this.content.appendChild(
        $$('.emails.contrib-data', {
          children: [
            $$('span.contrib-label', {text: 'For correspondence: '}),
            $$('span', {
              children: _.map(this.node.emails, function(email, index, array) {
                const node = $$('a', {href: "mailto:"+email, text: email});
                return array.length - 1 !== index ? [node, $$('span', {text: ';'})] : node;
              }).flat()
            })
          ]
        })
    ...
    if (this.node.fundings && this.node.fundings.length > 0) {
      this.content.appendChild(
    ...
    if (this.node.competing_interests && this.node.competing_interests.length) {
      this.content.appendChild(
    ...
    if (this.node.members && this.node.members.length > 0) {
      this.content.appendChild(
   ...
   // footnote label
    ...
    if (this.node.footnotes && this.node.footnotes.length > 0) {
